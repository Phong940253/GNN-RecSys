...
class DataPaths:
    def __init__(self):
        ...
        self.item_feat_path = './data/BX_Books.csv'
        self.user_feat_path = './data/BX-Users.csv'
...
class DataLoader:
    ...
    def __init__(self, data_paths, fixed_params):
        ...
        self.ctm_id, self.pdt_id = create_ids(
            self.user_item_train,
            # self.user_sport_interaction,
            # self.sport_sportg_interaction,
            self.item_feat_df,
...
def assign_graph_features(graph,
    ...
    if params['use_recency']:
        ...
        if fixed_params.discern_clicks:
            ...
            graph.edges['vote'].data['recency'] = recency_tensor_buys
            graph.edges['vote-by'].data['recency'] = recency_tensor_buys
            ...
            graph.edges['vote'].data['recency'] = recency_tensor
            graph.edges['vote-by'].data['recency'] = recency_tensor
    ...
    if fixed_params.duplicates == 'count_occurrence':
        # if fixed_params.discern_clicks:
            ...
            # graph.edges['vote'].data['occurrence'] = torch.tensor(data.adjacency_dict['purchases_num'])
            # graph.edges['vote-by'].data['occurrence'] = torch.tensor(data.adjacency_dict['purchases_num'])
        # else:
        graph.edges['vote'].data['occurrence'] = torch.tensor(data.adjacency_dict['user_item_num'])
        graph.edges['vote-by'].data['occurrence'] = torch.tensor(data.adjacency_dict['user_item_num'])
